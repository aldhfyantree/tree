...
let people=window.SEED_PEOPLE||[],remoteLoaded=false,currentPerson=null,userProfile=null,confirmation=null,zoom=1,radialState=null,selectedId=null,treeMode='radial',expandedIds=new Set(),zoomBehavior=null;
...
function renderRadialTree(){
  ...
  const dots=nodes.map(n=>{const main=n.depth===1,rootNode=n.id===root.id,displayName=rootNode?'ضفي':n.name,hasKids=(radialState.kids.get(n.id)||[]).length,open=expandedIds.has(n.id),w=rootNode?104:main?116:96;return `<g class="radial-person depth-${n.depth}${main?' main-branch':''}${rootNode?' root-person':''}${open?' is-open':''}" data-node="${n.id}" transform="translate(${n.x.toFixed(1)} ${n.y.toFixed(1)})"><rect class="person-card-bg" x="${-w/2}" y="-21" width="${w}" height="42" rx="14"/><text class="node-name" y="5" text-anchor="middle">${esc(displayName)}</text>${hasKids?`<circle class="branch-state" cx="${w/2-7}" cy="-15" r="9"/><text class="branch-state-mark" x="${w/2-7}" y="-11" text-anchor="middle">${open?'−':'+'}</text>`:''}</g>`}).join('');
...
function showPersonMenu(p){currentPerson=p;selectPerson(p.id);$('#menuPersonName').textContent=lineage(p);$('#personMenu').classList.remove('hidden')}
$('#tree').onclick=e=>{if(e.defaultPrevented)return;const n=e.target.closest('[data-node]');if(!n)return;const p=byId().get(n.dataset.node);if(!p)return;showPersonMenu(p);const hasKids=(childrenMap().get(p.id)||[]).length;if(hasKids){expandedIds.has(p.id)?expandedIds.delete(p.id):expandedIds.add(p.id);renderTree();showPersonMenu(p)}};
function bindRadialGestures(){const svg=$('#familySvg');if(!svg||!window.d3)return;zoomBehavior=d3.zoom().scaleExtent([.45,4]).clickDistance(8).on('zoom',event=>{d3.select('#radialGraph').attr('transform',event.transform);zoom=event.transform.k;updateRadialVisibility();$('#zoomLabel').textContent=`${ar(Math.round(zoom*100))}٪`});d3.select(svg).call(zoomBehavior).on('dblclick.zoom',null)}
function fitTree(){const svg=$('#familySvg');if(!svg||!zoomBehavior)return;d3.select(svg).transition().duration(220).call(zoomBehavior.transform,d3.zoomIdentity)}
...
function resetTree(){const root=people.find(x=>!x.parent);expandedIds=new Set(root?[root.id]:[]);zoom=1;selectedId=null;$('#personMenu').classList.add('hidden');$('#dataStatus').textContent='الفروع الرئيسية';renderTree()}
...
$('#fitTreeBtn').onclick=fitTree;$('#zoomIn').onclick=()=>buttonZoom(1.3);$('#zoomOut').onclick=()=>buttonZoom(1/1.3);function buttonZoom(f){const svg=$('#familySvg');if(!svg||!zoomBehavior)return;d3.select(svg).transition().duration(160).call(zoomBehavior.scaleBy,f)}
...
$('#copyLineageBtn').onclick=async()=>{await navigator.clipboard.writeText(lineage(currentPerson));toast('تم نسخ الاسم الكامل')};
